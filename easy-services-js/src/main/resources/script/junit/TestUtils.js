...
var jsAssert = {};
var JSTest = Packages.org.zenframework.easyservices.js.junit.JSTest;
	...
	return {
		...
		assertCalled : function(description) {
			...
			if (this.called.length < 1)
				assert.fail('No functions called, expected: ' + fnDescToString(description));

			for (var i = 0; i < this.called.length; i++) {
				var fn = this.called[i];
				if (fn.name == description.name) {
					if (description.args.length != fn.args.length)
						continue;

					for (var j = 0; j < description.args.length; j++) {
						if (fn.args[j] == description.args[j])
							return;
					}
				}
			}

			assert.fail('No matching functions called. expected: ' + '<' + fnDescToString(description) + ")>" + ' but had '
...
var tests = function(testObject) {
	var jsTests = new java.util.ArrayList();
	for ( var name in testObject) {
		if (testObject.hasOwnProperty(name)) {
			jsTests.add(new JSTest(name, testObject[name]));
		}
	}
	return jsTests;
};
